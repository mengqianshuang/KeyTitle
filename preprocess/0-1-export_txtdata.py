...
def main(solution, jsonfile):
    ...
    for idx, issue in tqdm(enumerate(valid_issues)):
        if idx % 50000 == 0:
    ...
    with open("body.train.ap.txt", "w", encoding="utf8") as fbody, open("title.train.ap.txt", "w",encoding="utf8") as ftitle:
        bodies = [x['body'] + "\n" for x in valid_issues_train]
        titles = [x['title'] + "\n" for x in valid_issues_train]

        fbody.writelines(bodies)
        ftitle.writelines(titles)
    with open("body.valid.ap.txt", "w", encoding="utf8") as fbody, open("title.valid.ap.txt", "w", encoding="utf8") as ftitle:
        bodies = [x['body'] + "\n" for x in valid_issues_val]
        titles = [x['title'] + "\n" for x in valid_issues_val]
        fbody.writelines(bodies)
        ftitle.writelines(titles)
    with open("body.test.ap.txt", "w", encoding="utf8") as fbody, open("title.test.ap.txt", "w", encoding="utf8") as ftitle:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-file', dest='jsonfile', default="refined_issues_ap.json",
