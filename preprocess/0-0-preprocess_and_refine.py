...
def improve_body(origin_str, return_max_cnt = False): 
    ...
    origin_str = search.sub(" CODE ", origin_str)
    ...
    if return_max_cnt:
        maxcnt = max(maxcnt, len(search.findall(origin_str)))
    origin_str = search.sub(" URL ", origin_str)
...
def main():
    ...
    all_issues = pd.read_csv("../data/eclipse.csv")
    ...
    print ("preprocessing...")
    preprocessed_issues = pd.DataFrame(data=None, columns=['summary', 'text'])
    num = 0
    for idx, issue in tqdm(all_issues.iterrows()):
        # if idx % 10000 == 0:
        #     print ("current idx:", idx, "/", len(all_issues))

        # substitute miscellaneous info in body with placeholder.
        try:
            issue_text, _ = improve_body(issue['text'])

            # remove issues with improper body: improper length OR with HTML tag outside markdown code field.
            issue_body_tokenize = nltk.word_tokenize(issue['text'])
            if filter_body(issue['text'], issue_body_tokenize):
                continue

            # remove "tag: " and "[tag]" from title
            issue_sum = improve_title(issue['summary'])

            preprocessed_issues.loc[num] = [issue_sum, issue_text]
            num += 1
        except:
            continue
...
#         all_issues = json.load(f) 
    ...
    valid_issues_train = pd.DataFrame(data=None, columns=['summary', 'text'])
    valid_issues_val = pd.DataFrame(data=None, columns=['summary', 'text'])
    valid_issues_test  = pd.DataFrame(data=None, columns=['summary', 'text'])

    num1, num2, num3 = 0, 0, 0
    for idx, issue in tqdm(preprocessed_issues.iterrows()):
        # if idx % 10000 == 0:
        ...
        try:
            issue_body_tokenize = nltk.word_tokenize(issue['text'])
            issue_title_tokenize = nltk.word_tokenize(issue['summary'])
            ...
            if (rule1checker(issue['text'], issue_title_words)) \
             or(rule2checker(issue_title_words, issue_body_words)) \
             or(rule3checker(issue['summary'], issue_title_tokenize, issue_body_tokenize)): # can disable any rule for ablation
                continue
            # preprocessed_issues.loc[num] = [issue_sum, issue_text]
            if idx < sep1:
                valid_issues_train.loc[num1] = [issue['summary'], issue['text']]
                num1 += 1
            elif idx < sep2:
                valid_issues_val.loc[num2] = [issue['summary'], issue['text']]
                num2 += 1
            else:
                valid_issues_test.loc[num3] = [issue['summary'], issue['text']]
                num3 += 1
        except:
            continue
    ...
    valid_issues = pd.concat([valid_issues_train, valid_issues_val, valid_issues_test], axis=0, ignore_index=True)
    ...
    valid_issues.to_csv("../data/itape_eclipse.csv", index=False)
    valid_issues_train.to_csv("../data/valid_eclipse_train.csv", index=False)
    valid_issues_val.to_csv("../data/valid_eclipse_val.csv", index=False)
    valid_issues_test.to_csv("../data/valid_eclipse_test.csv", index=False)
